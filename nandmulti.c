...
void nandMul(uint64_t *in1, uint64_t *in2, uint64_t *out1, uint64_t *out2) {
    if (*in1 + *in2 == 0) {
        return;}
    for (int i = 0; i < 2; i++) {
        asm volatile(
    ...
    for (int i = 0; i < NUM_TRIALS; i++) {
        ...
        *out2 = 0;
        ...
        if (should_cache == 0) {
            ...
            set(in2);
            _mm_mfence();
            nandMul(in1, in2, out1, out2);
            int a = test(out1);
            int b = test(out2);
            if (a==0 && b==0) success1++;
            testCached++; 
        } else if (should_cache == 1) {
            clear(in1);
            clear(in2);
            nandMul(in1, in2, out1, out2);
            if (test(out2)) success0++;
            testUncached++;   
