...
typedef struct {
    unsigned trials;          // how many times this exact 6-bit pattern appeared
    unsigned wrong[3];        // wrong counts per output bit (0..2)
    unsigned wrong_both;      // count cases where >=2 bits were wrong
} PatternStats;

static PatternStats patstats[64]; // 2^6 input patterns

static inline int pattern_index_from_should_cache(const int sc[6]) {
    // sc = {a0,a1,a2,b0,b1,b2} in your code's order? You currently fill should_cache[0..5]
    // You use mod_pairs( [0]+[3], [1]+[4], [2]+[5] ), so index as [a0 a1 a2 b0 b1 b2]
    // Build bitstring: a2 a1 a0 b2 b1 b0 (just pick a consistent print order)
    int idx = 0;
    idx |= (sc[0] & 1) << 0; // a0
    idx |= (sc[1] & 1) << 1; // a1
    idx |= (sc[2] & 1) << 2; // a2
...
static void print_bits6(int idx) {
    // print as {a2,a1,a0,b2,b1,b0} for readability
    int a0 = (idx >> 0) & 1;
    int a1 = (idx >> 1) & 1;
    int a2 = (idx >> 2) & 1;
    int b0 = (idx >> 3) & 1;
    int b1 = (idx >> 4) & 1;
    int b2 = (idx >> 5) & 1;
    printf("{%d,%d,%d,%d,%d,%d}", a2, a1, a0, b2, b1, b0);
}
...
int main() {
    ...
    for (int i = 0; i < NUM_TRIALS; i++) {
        ...
        *in4 = 1;
        ...
        for (int i = 0; i < 500; i++) {
            nand(in3, in4, sum_out[1]);
            not(in3, sum_out[1]);
            nandMul(in3, in4, sum_out[1], sum_out[0]);
        }

        halfadder_all(in1[0], in2[0], in1copy[0], in2copy[0], tmp1[0], tmp2[0], tmp3[0], tmp4[0], sum_out[0], should_cache[0], should_cache[3], all_zero, res, 0);
        halfadder_all(in1[1], in2[1], in1copy[1], in2copy[1], tmp1[1], tmp2[1], tmp3[1], tmp4[1], sum_out[1], should_cache[1], should_cache[4], all_zero, res, 1);
        halfadder_all(in1[2], in2[2], in1copy[2], in2copy[2], tmp1[2], tmp2[2], tmp3[2], tmp4[2], sum_out[2], should_cache[2], should_cache[5], all_zero, res, 2);
        
        // --- instrumentation update ---
        int pidx = pattern_index_from_should_cache(should_cache);
        patstats[pidx].trials++;

        int wrong_count = 0;
        if (res[0] != wanted_res[0]) { patstats[pidx].wrong[0]++; wrong_count++; }
        if (res[1] != wanted_res[1]) { patstats[pidx].wrong[1]++; wrong_count++; }
        if (res[2] != wanted_res[2]) { patstats[pidx].wrong[2]++; wrong_count++; }
        if (wrong_count >= 2) patstats[pidx].wrong_both++;
        ///end
    ...
    printf("\n=== Failure by 6-bit input pattern ===\n");
    printf("Pattern {a2,a1,a0,b2,b1,b0}  Trials  Wrong(bit0/1/2)  >=2 wrong\n");
    for (int i = 0; i < 64; i++) {
        if (patstats[i].trials == 0) continue;
        printf("  "); print_bits6(i);
        printf("   %5u     %5u/%5u/%5u     %5u\n",
               patstats[i].trials,
               patstats[i].wrong[0],
               patstats[i].wrong[1],
               patstats[i].wrong[2],
               patstats[i].wrong_both);
    }

    // Find top offenders per bit:
    for (int bit = 0; bit < 3; bit++) {
        // simple selection of worst top-5
        int top_idx[5] = {-1,-1,-1,-1,-1};
        for (int i = 0; i < 64; i++) {
            if (patstats[i].trials == 0) continue;
            for (int k = 0; k < 5; k++) {
                if (top_idx[k] < 0 ||
                    patstats[i].wrong[bit] > patstats[top_idx[k]].wrong[bit]) {
                    // shift down
                    for (int s = 4; s > k; s--) top_idx[s] = top_idx[s-1];
                    top_idx[k] = i;
                    break;
                }
            }
        }
        printf("\n=== Top 5 failing patterns for bit%d ===\n", bit);
        for (int k = 0; k < 5; k++) {
            int i = top_idx[k];
            if (i < 0) continue;
            double err_rate = (patstats[i].trials ? 
                (100.0 * patstats[i].wrong[bit] / patstats[i].trials) : 0.0);
            printf("  "); print_bits6(i);
            printf("  wrong=%u/%u (%.1f%%)\n",
                   patstats[i].wrong[bit], patstats[i].trials, err_rate);
        }
    }
    return 0;
}
